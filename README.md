...
Planned future usage of Blaster is as follows for the following commands `run` and `generate`,
which run a load test and generate seed data for endpoints requiring data-backed parameters respectively.
...
./stellar-rpc-loadtest generate \
  ...
  [--count NUM_LEDGERS]
  ```
Note that for either command, should one wish to run Blaster on a localhost RPC testnet instance,
they may use `--rpc-url "http://127.0.0.1:8000"` (note that instance must be running separately).
